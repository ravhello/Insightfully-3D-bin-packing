...
def rectIntersect(item1, item2, x, y):
    ...
    cx1 = item1.position[x] + d1[x] / Decimal('2')
    cy1 = item1.position[y] + d1[y] / Decimal('2')
    cx2 = item2.position[x] + d2[x] / Decimal('2')
    cy2 = item2.position[y] + d2[y] / Decimal('2')

    ix = max(cx1, cx2) - min(cx1, cx2)
    iy = max(cy1, cy2) - min(cy1, cy2)

    return ix < (d1[x] + d2[x]) / Decimal('2') and iy < (d1[y] + d2[y]) / Decimal('2')
...
def set2Decimal(value, number_of_decimals=0):
    ...
    return value.quantize(number_of_decimals)
